...
fn main() -> ResultType<()> {
    ...
    let args = format!(
        ...
        -u, --software-url=[URL] 'Sets download url of CscpAssist software of newest version'
        -i, --intermediary-servers=[HOST] 'Sets the default intermediary servers, separated by colon'
        -M, --rmem=[NUMBER(default={RMEM})] 'Sets UDP recv buffer size, set system rmem_max first, e.g., sudo sysctl -w net.core.rmem_max=52428800. vi /etc/sysctl.conf, net.core.rmem_max=52428800, sudo sysctl –p'
        , --mask=[MASK] 'Determine if the connection comes from LAN, e.g. 192.168.0.0/16'
        -k, --key=[KEY] 'Only allow the client with the same key'",
    );
    init_args(&args, "cscpassist-controller", "CscpAssist ID/Rendezvous Server");
